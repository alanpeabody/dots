" Name:    cyberpunk2018
" Remixer: alanpeabody
" About: Cyberpunk colors in the vim-one color file structure
" Vim One Author (code):  Ramzi Akremi
" Vim One License (code): MIT
" Cyberpunk Author (colors): max-SS
" Cyberpunk License (colors): GPL v3.0
...
let g:colors_name = 'cyberpunk2018'

"if !exists('g:cp2018_allow_italics')
  "let g:cp2018_allow_italics = 0
  let g:cp2018_allow_italics = 1
"endif
...
if has('gui_running') || has('termguicolors') || &t_Co == 88 || &t_Co == 256
  ...
  fun <sid>X(group, fg, bg, attr)
    let l:attr = a:attr
    if g:cp2018_allow_italics == 0 && l:attr ==? 'italic'
      let l:attr= 'none'
    endif
  ...
  " Color definition --------------------------------------------------------{{{
  let s:dark = 1
  let s:mono_1 = 'e5e5e5'  " light grey
  let s:mono_2 = '826ab0'  " light purple (comments)
  let s:mono_3 = '68548c'  " less light purple
  let s:mono_4 = '332a57'  " purple

  let s:hue_1  = '86cbfe' " cyan
  let s:hue_2  = '00bfff' " blue
  let s:hue_3  = 'df95ff' " purple
  let s:hue_4  = '00fbac' " green

  let s:hue_5 = 'ff8aa4' " red 2
  let s:hue_5_2   = 'ff7092' " red 1

  let s:hue_6 = 'fff787' " yellow 2
  let s:hue_6_2   = 'fffa6a' " yellow 1


  let s:syntax_bg     = '12OD24' " dark purple
  let s:syntax_gutter = '332A57' " medium purple
  let s:syntax_cursor = '090517' " extra dark purple
  let s:syntax_accent = '528bff' " lightish blue

  let s:vertsplit    = '0A261C' " greenscreen bg
  let s:pmenu        = '0A261C' " greenscreen bg
  let s:pmenu_fg     = '00FF6F' " greenscreen fg
  ...
  call <sid>X('CursorLineNr', s:mono_3,        s:syntax_cursor,  'none')
  call <sid>X('MatchParen',   s:hue_5,         s:syntax_cursor,  'underline,bold')
  call <sid>X('Italic',       '',              '',               'italic')
  call <sid>X('ModeMsg',      s:pmenu_fg,     '',               '')
  call <sid>X('MoreMsg',      s:pmenu_fg,     '',               '')
  call <sid>X('NonText',      s:mono_3,        '',               'none')
  call <sid>X('PMenu',        s:pmenu_fg,      s:pmenu,          '')
  call <sid>X('PMenuSel',     s:pmenu,         s:pmenu_fg,      '')
  call <sid>X('PMenuSbar',    '',              s:pmenu,          '')
  call <sid>X('PMenuThumb',   '',              s:pmenu,         '')
  call <sid>X('Question',     s:hue_2,         '',               '')
  call <sid>X('Search',       s:syntax_bg,     s:mono_2,        '')
  call <sid>X('SpecialKey',   s:mono_4,        '',               'none')
  call <sid>X('Whitespace',   s:mono_4,        '',               'none')
  call <sid>X('StatusLine',   s:pmenu_fg,      s:pmenu,          'bold')
  call <sid>X('StatusLineNC', s:pmenu,         s:pmenu_fg,       '')
  call <sid>X('TabLine',      s:pmenu_fg,      s:pmenu,          'none')
  call <sid>X('TabLineFill',  s:pmenu_fg,      s:pmenu,          'none')
  call <sid>X('TabLineSel',   s:pmenu,         s:pmenu_fg,       '')
  call <sid>X('Title',        s:syntax_fg,     '',               'bold')
  call <sid>X('Visual',       '',              s:mono_4,    '')
  call <sid>X('VisualNOS',    '',              s:mono_4,    '')
  call <sid>X('WarningMsg',   s:hue_5,         '',               '')
  call <sid>X('TooLong',      s:hue_5,         '',               '')
  call <sid>X('WildMenu',     s:pmenu_fg,      s:pmenu,          '')
  call <sid>X('SignColumn',   '',              s:pmenu,      '')
  ...
  " Diff highlighting -------------------------------------------------------{{{
  call <sid>X('DiffAdd',     s:hue_4, '', '')
  call <sid>X('DiffChange',  s:hue_6, '', '')
  call <sid>X('DiffDelete',  s:hue_5, '', '')
  call <sid>X('DiffText',    s:hue_2, '', '')
  call <sid>X('DiffAdded',   s:hue_4, '', '')
  call <sid>X('DiffFile',    s:hue_5, '', '')
  call <sid>X('DiffNewFile', s:hue_4, '', '')
  call <sid>X('DiffLine',    s:hue_2, '', '')
